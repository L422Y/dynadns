...
module.exports = class dynadns {
    ...
    setupLogging() {
        ...
        if (this.opts.DEBUG && !this.ISDAEMON) {
            ...
            // rotate logs
            if (fs.existsSync('dynadns.log.3')) fs.unlinkSync('dynadns.log.3')
            if (fs.existsSync('dynadns.log.2')) fs.renameSync('dynadns.log.2', 'dynadns.log.3')
            if (fs.existsSync('dynadns.log.1')) fs.renameSync('dynadns.log', 'dynadns.log.2')
            if (fs.existsSync('dynadns.log')) fs.renameSync('dynadns.log', 'dynadns.log.1')

            let access = fs.createWriteStream('dynadns.log', { flags: 'a+' })
            l = (...args) => {
                access.write([`[${new Date().toISOString()}]`, "[INFO]", ...args].join(' ') + "\n")
            }
            le = (...args) => {
                access.write([`[${new Date().toISOString()}]`, "[ERROR]", ...args].join(' ') + "\n")
            }
